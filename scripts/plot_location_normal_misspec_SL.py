...
if model == "normal_location_misspec":
    methods_list = ["Standard Bayes", "SyntheticLikelihood"]
    n_params = 1
    extract_par_fcn = extract_params_from_journal_normal
    outliers_list = [1, 3.0, 5.0]
    ...
    for epsilon in epsilon_list:
        ...
        if method != "Standard Bayes":
            ...
            filename = inference_folder + f"{method}_MCMC_burnin_{burnin}_n-samples_{n_samples}_n-sam-per-param_" \
                                          f"{n_samples_per_param}_n-sam-in-obs_{n_samples_in_obs}_epsilon_{epsilon}_outliers_loc_{actual_outlier}"
            ...
            post_samples = np.load(true_posterior_folder + f"n-samples_{10000}_burnin_{10000}" \
                                                           ...
                                                           f"_epsilon_{epsilon}_outliers_loc_{actual_outlier}.npy")
...
axes[0, 0].set_ylabel(r"Generalized posteriors" "\n" "$z={}$".format(int(outlier)))  # add legend in one panel only
...
for method_idx, method in enumerate(methods_list):
    ...
    for outlier_idx, outlier in enumerate(outliers_list):
        ...
        if method != "Standard Bayes":
            # load journal
            filename = inference_folder + f"{method}_MCMC_burnin_{burnin}_n-samples_{n_samples}_n-sam-per-param_" \
                                          f"{n_samples_per_param}_n-sam-in-obs_{n_samples_in_obs}_epsilon_{actual_epsilon}_outliers_loc_{outlier}"
            journal = Journal.fromFile(filename + ".jnl")
            ...
            post_samples = np.load(true_posterior_folder + f"n-samples_{10000}_burnin_{10000}" \
                                                           f"_n-sam-per-param_{n_samples_in_obs}" \
                                                           f"_epsilon_{actual_epsilon}_outliers_loc_{outlier}.npy")
...
plt.savefig(inference_folder + f"Overall_plot_burnin_{burnin}_n-samples_{n_samples}_n-sam-per-param_"
                               f"{n_samples_per_param}_thinning_{thin}_SL.pdf")

# PLOT TYPE 2

fig, axes = plt.subplots(figsize=((len(outliers_list) - 1) * 3, (len(epsilon_list) - 1) * 2.5),
                         nrows=len(epsilon_list) - 1, ncols=len(outliers_list) - 1, sharex="col", sharey="none")
    ...
    for outlier_index, outlier in enumerate(outliers_list[1:]):
        ...
        for method_idx, method in enumerate(methods_list):
            ...
            if method != "Standard Bayes":
                # load journal
                filename = inference_folder + f"{method}_MCMC_burnin_{burnin}_n-samples_{n_samples}_n-sam-per-param_" \
                                              f"{n_samples_per_param}_n-sam-in-obs_{n_samples_in_obs}_epsilon_{epsilon}_outliers_loc_{outlier}"
                journal = Journal.fromFile(filename + ".jnl")
                # print("Std dev", np.sqrt(journal.posterior_cov()[0]))
                ...
                post_samples = np.load(true_posterior_folder + f"n-samples_{10000}_burnin_{10000}" \
                                                               f"_n-sam-per-param_{n_samples_in_obs}" \
                                                               f"_epsilon_{epsilon}_outliers_loc_{outlier}.npy")
                # print("Std dev", np.std(post_samples.reshape(-1)))
...
plt.savefig(inference_folder + f"Overall_plot_burnin_{burnin}_n-samples_{n_samples}_n-sam-per-param_"
                               f"{n_samples_per_param}_thinning_{thin}_2_SL.pdf")

# PLOT TYPE 3 (same as 2 but with traces)
        ...
        for method_idx, method in enumerate(methods_list):
            ...
            if method != "Standard Bayes":
                ...
                post_samples = np.load(true_posterior_folder + f"n-samples_{10000}_burnin_{10000}" \
                                                               f"_n-sam-per-param_{n_samples_in_obs}" \
                                                               f"_epsilon_{epsilon}_outliers_loc_{outlier}.npy")
            axes[eps_index, outlier_index].plot(post_samples, label=method)
            axes[eps_index, outlier_index].axhline(theta_obs[0], color="green", ls="dashed")

            axes[eps_index, outlier_index].set_title(r"$\epsilon={}, z = {}$ ".format(epsilon, int(outlier)))

for outlier_index, outlier in enumerate(outliers_list[1:]):
    axes[-1, outlier_index].set_xlabel("MCMC step")

for eps_index, epsilon in enumerate(epsilon_list[1:]):
    axes[eps_index, 0].set_ylabel(param_names_latex[0])

# axes[-1, -1].legend()
plt.savefig(inference_folder + f"Overall_plot_burnin_{burnin}_n-samples_{n_samples}_n-sam-per-param_"
                               f"{n_samples_per_param}_thinning_{thin}_3_SL.pdf")
